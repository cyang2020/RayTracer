...
Ray Camera::generate_ray_for_thin_lens(double x, double y, double rndR, double rndTheta) const {
    ...
    Vector3D bottomLeft = Vector3D(-tan(radians(hFov) * 0.5), -tan(radians(vFov)*.5), -1);
    Vector3D topRight = Vector3D(tan(radians(hFov) * 0.5),  tan(radians(vFov) * 0.5), -1);
    
    double w = topRight.x - bottomLeft.x;
    double h = topRight.y - bottomLeft.y;

    Vector3D d = Vector3D(bottomLeft.x + w * x, bottomLeft.y + h * y , -1);
    Vector3D pFocus = d * ((-1.0 * focalDistance) / (d.z));
    
    Ray r = Ray(pLens, (pFocus - pLens));
